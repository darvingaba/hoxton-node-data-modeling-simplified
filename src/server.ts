...
`);
const getInterviewersForCompanies = db.prepare(`
SELECT name FROM interviewers
JOIN employers ON interviewers.id = employers.interviewerId
WHERE employers.companyId=@companyId
`);
const getPassedInterviews = db.prepare(`
SELECT name FROM applicants
JOIN interviews ON applicants.id = interviews.applicantId
WHERE interviews.interviewerId=@interviewerId AND interviews.successful ='Yes'
`);
